...
const Index = () => {
  ...
  const addPost = () => {
    if (newPost.trim() !== "") {
      addPostMutation.mutate({ title: newPost, body: newPost, author_id: user?.id || 'anonymous' });
      setNewPost("");
    }
  };

  const addReaction = (postId, emoji) => {
    addReactionMutation.mutate({ post_id: postId, user_id: user?.id || 'anonymous', emoji });
  };
